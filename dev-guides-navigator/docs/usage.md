...
It is a routing and caching layer over a published catalog of guides, so an AI session works from current best practice instead of a training-time guess. Six modes, over separate catalogs, and the caller (typically an orchestrator like `ai-dev-assistant`, or you directly) decides which to try first:
