...
- **A git repository of its own.** Every project gets one. It tracks text: the state file, the
  notes, and the small records each stage writes. It ignores everything else by default, so a new
  kind of file is left out, not silently let in. If something got ignored that should have been
  tracked, the check names it.
...
- which recipe each stage adopted
- whether visual regression or end-to-end testing is set up
- whether a memory hook is installed

None of this is asked at creation. Each field is filled in by the stage that needs it, the first
time it needs it.
...
Switch looks the target up in AIDA's own list and, if found, loads that project into the current
...
report, create, or switch a project. You can also run it directly against a project folder.
...
A stage that needs a field the check flagged as missing can trigger that one repair on the spot.
You do not have to run the check yourself first.
...
reports what is missing instead of guessing at a path or a stack.
...
own source for guides, playbooks, or process recipes: a local folder, or a site your team prefers.
Where it does, its own source wins over the catalog.

You can set this per kind of content, and even per stage. A team's own method for one stage can
stand alongside the catalog's answer for every other stage.

The project's state file already reserves the place for this: an ordered list of sources. At
creation it holds one entry, the hosted catalog, plus a way to override that order per stage and
framework.

**The screen that lets you add your own source is not built yet.** Until it is, every project
reads from the catalog only. Creating and switching a project will not change when that screen
arrives.
