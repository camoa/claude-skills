...
# or unparseable store answers as if it were empty, the same way project-for-cwd.sh in version 5
# treated a missing registry as "unregistered" rather than an error.
#
...
# Emits the store's current content. Never writes. A missing or unparseable store answers as the
# empty skeleton rather than an error — the same fallback version 5's readers used for a first-ever
# run.
registry__current() {
...
registry_add_project() {
  ...
  if registry__current | jq -e --arg c "$codepath" \
  ...
  local now new
  now="$(date -u +%Y-%m-%d)"
  new="$(registry__current | jq --arg c "$codepath" --arg p "$projectpath" --arg t "$now" '
    .version = 1
...
registry_record_declined_offer() {
  ...
  local now new
  now="$(date -u +%Y-%m-%d)"
  new="$(registry__current | jq --arg d "$directory" --arg t "$now" '
    .version = 1
    | .declinedOffers = (
...
registry_touch_last_accessed() {
  ...
  if ! registry__current | jq -e --arg p "$projectpath" \
  ...
  local now new
  now="$(date -u +%Y-%m-%d)"
  new="$(registry__current | jq --arg p "$projectpath" --arg t "$now" '
