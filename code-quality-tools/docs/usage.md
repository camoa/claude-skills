...
- **[claude-plugin-checks](../../claude-plugin-checks/README.md)** and **[code-paper-test](../../code-paper-test/README.md)** cover Claude Code plugin structure and behavioral verification respectively; this plugin does not check plugin/skill files itself, it checks the application code (Drupal, Next.js) those plugins are built to work on.
