...
Renamed from `plugin-creation-tools`, reduced to the part worth keeping, and then
rebuilt as one program after a fresh-context review found five defects that reported a
clean plugin.

### The five that reported clean

- A colon anywhere in the scanned path made the containment scan drop every finding.
  It split each grep row on `:`, the line number came out non-numeric, `jq` errored to
  stderr and the record vanished. A tree with a home path and an AWS key in it passed.
- Each category took the first match on a line and skipped the line if that one match
  was a documentation placeholder, so `/home/user/a` earlier on a line hid a real
  username later on it. A tutorial line has exactly that shape.
- The leading boundary class excluded `/`, so a `file://` URL naming a home directory
  never matched.
- A manifest string counted as a declared path only when it started with `./`. An
  absolute path and the `${CLAUDE_PLUGIN_ROOT}` form were both invisible, so a manifest
  pointing a component at `/etc/passwd` reported clean, and across the nine plugins in
  this repository the containment and existence rules fired on nothing at all.
- The runner exited 0 when five of its six checks could not look, because one passing
  check satisfied its "did anything run" test.

### Changed: one program, not three scripts

The three standalone scripts are gone. `scripts/check.py` is the entry point over
`scripts/cpc/`, where `result.py` owns the output shape, the pass/fail/could-not-look
vocabulary and the exit codes, `walk.py` owns the one tree walk, `manifest.py` owns
manifest reading and path resolution, and `rules/` holds one module per family that
adds findings and decides nothing else.

The split was not tidiness. Each old script walked the tree itself, and the review
found the same defect in all three: a directory that could not be read silently shrank
what got checked. One walk, one fix. The runner's exit-code bug only existed because
there were six results to reduce into one.

A rule is now unit-testable as a function. 71 tests call rules directly; a separate
13-assertion spec covers what a caller reading only an exit code is told.

### Changed: exit codes mean the same thing everywhere

`0` everything examined and clean, `1` a finding, `2` bad arguments, `3` something
could not be looked at. Zero requires that everything looked, so a machine without the
`claude` CLI does not get one. The previous version documented a could-not-look code
that the containment scan had no path to produce.

### Fixed after the rewrite, by running it rather than testing it

- Dropping the boundary class to catch `file://` made `snapshots/home/screen.snap`
  match. The rule now asks whether the match starts a path, and a `/` before it counts
  only as the second slash of a scheme.
- Any manifest string containing a slash was collected as a path, so a description
  reading "deeper SOLID/DRY analysis" was reported as a missing component. A declared
  path has no whitespace in it.
- Reporting every binary file as a blind spot made any plugin with a screenshot
  permanently unclean. They are counted instead, and the count stays visible.
- A control character in a scanned file reached the JSON and made the whole result
  unparsable. Everything on the way out is sanitised.
- `spec@example.invalid` was reported as a personal address. `.invalid`, `.example`
  and `.localhost` are reserved for documentation the same way `.test` is.

### Note on what a finding is allowed to say

A byte-order mark before a frontmatter fence is a warning, not an error, and the note
does not claim the loader ignores the block. The grant-and-denial finding names the
collision without saying which side wins. Both follow the rule that killed the
predecessor: if nobody here has measured it, the finding does not assert it.



Renamed from `plugin-creation-tools`, and reduced to the part that was worth keeping.

### Removed

- The 56-file reference tree, the templates, the two worked examples, the four Python
  scripts, the `plugin-creation` skill, both agents, and the `create`, `add-component`
  and `validate` commands.
- The 54-rule `/validate` command. Most of its rules encoded a fact Anthropic owns and
  can change: hook event names, model tiers, reserved marketplace names, manifest key
  allowlists, character caps, enum members. Each was correct when written and became a
  maintenance subscription with no expiry.
- The `PreCompact` hook and the `.claude/rules/` authoring conventions, both of which
  existed to support the teaching surface.

### Fixed in review, before shipping

A fresh reader ran the rewrite against fixtures and against the nine plugins here and
found ten more defects. Every one of these reported a clean plugin:

- One warning anywhere in a run turned an unreadable directory into a pass. The
  blind-spot test was "blind AND no findings", so a tree with a credential behind a
  permission-denied directory came back clean with exit 0. A blind spot outranks a
  clean-looking remainder now, always.
- A missing PyYAML disabled the whole self-consistency module, and the warning above
  hid it. An agent granting and denying `Write` in one block exited 0.
- A symlinked directory contributed no files, no counter and no blind spot, so a
  credential behind a vendored folder was invisible. The walk follows links and guards
  against a loop.
- One NUL byte in the first 8KB skipped a text file whole. Binary is decided by how
  much of the sample is printable, not by a single byte.
- A YAML comment, a blank line, a quoted key or a block scalar in frontmatter made a
  real component body look like prose and switched every self-consistency rule off for
  that file. The block is handed to the YAML parser now rather than matched line by
  line against one narrow shape.
- The unterminated-fence rule only saw files a manifest declares by path, and no plugin
  here declares any, so it fired on nothing at all. It moved to the module that walks
  every markdown file.
- A repository shorthand, a homepage and a keyword were all collected as component
  paths and reported missing. Only the explicit forms say they are paths on their own;
  a bare relative string is settled by asking whether anything is there.
- The plugin did not pass its own check: a fixture in its own spec carried a
  leak-shaped literal. It is assembled at run time, and `.containment-allow` now ships
  empty.
- A docs page with two horizontal rules in it was reported as misplaced frontmatter,
  fourteen times in one plugin. That was a regression from the frontmatter fix above.
- The AWS key that appears in AWS's own documentation was reported as a credential.

### Added

- `skills/check` — one skill that runs everything available and prints what each tool
  said, unfiltered. It runs `claude plugin validate`, `plugin-dev`'s hook linter and
  agent validator, and `check.py`.
- `scripts/run-checks.sh` — the runner behind it. Resolves `plugin-dev` by glob rather
  than a written-down path, collects files without `mapfile` so a check cannot vanish
  on an older shell, and names the two `plugin-dev` scripts it deliberately does not
  run: `validate-hook-schema.sh` crashes on the standard `hooks.json` shape, and
  `validate-settings.sh` reads a settings file rather than a plugin.
- Containment gained Stripe's secret prefixes. The shipped pattern was `sk-` with a
  hyphen and Stripe writes `sk_live_` with an underscore, so `sk_`/`rk_` live and test
  keys and `whsec_` webhook secrets all passed. A publishable `pk_` key is not a secret
  and stays unflagged.
- `--only` on `check.py`, taking rule prefixes or identifiers, so one family can be run
  without three entry points existing.
