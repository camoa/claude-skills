...
| `security` | `security-report.json` | `unmeasured`, or `meta.analyzers_ran == 0` — the producer stating no scanner measured anything. **`meta.tools[]` is not read and must not be**: it exists only in whole-project mode, and `--changed` is `/review`'s default path, so a set relation over it is undefined on the path almost every review takes. Its names also disagreed with the ones the code pushed until cqt 3.10.4; that half is fixed, the mode half is not, and one is enough |
