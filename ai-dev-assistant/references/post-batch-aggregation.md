...
echo "$PAYLOAD" | jq -r '
  ...
  | select(length > 0)
  | "ai-dev-assistant team batch — \(length) teammate output(s):",
...
- A plugin-scoped `PostToolBatch` hook fires on **every** batch in **every** Claude Code conversation while ai-dev-assistant is enabled — including conversations that never touch `/research-team` or `/validate:team`. That is pure noise for the common case.
