...
- **`--dry-run`:** do NOT invoke the loader (it writes). Report "would sweep phases [research, design, implement, review, e2e-setup, visual-regression] across frameworks [<list>] and record resolved `source=` lines into project_state.md, no writes". Use the frameworks Step 4 would have set: the slugs the cascade in `references/framework-resolution.md` names from `scripts/framework-evidence.sh` output, read-only and with no prompt, when `**Frameworks:**` is not yet on disk. Exit 0.
