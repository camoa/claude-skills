...
| `components[]` | yes | One row per component actually critiqued. May be empty when `verdict` is `skipped`. Each row carries `runtime` (v5.35.2+): `executed` \| `static_only` \| `not_run`, with a non-empty `runtime_reason` for the last two. A row that cannot say whether its code ever ran is fail-closed. **A row for a component that was REPAIRED also carries `accept` (v5.42.0+)**: `{action, suite, decided_by, reason}`, the verdict `scripts/repair-accept-check.sh` returned over the repaired tree. `action` is `accepted` \| `not_accepted` \| `cannot_judge`; `suite` is the `green` \| `red` \| `not_run` result the builder handed the kernel, not one the gate ran; `decided_by` is `suite_and_motion` \| `motion` \| `none`; `reason` says what drove the action. Repaired is read two ways, a `rounds` count above 1 on the row or a `rounds[]` entry naming the component whose `round` is above 1 -- round 1 is the initial critique, not a repair, on either reading. **Absent on a repaired component is fail-closed**, and so is an `action` outside the enum. So is any of the other three fields being absent or off-enum: a verdict that cannot say what suite result it weighed, what settled it, or why, is unreadable rather than clean. So is a repair state nobody can establish -- a non-numeric `rounds` count with no `accept`, or a `rounds[]` entry naming such a component with no numeric `round`. Any `action` other than `accepted` then requires the decision in `escalation` below. |
