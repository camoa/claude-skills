...
| `frameworks[].arch_validate` | object \| null | **v5.35.7+, `phase: "review"` only.** The step-5.0 `architecture-validator` outcome for this framework: `{verdict, sidecar, reason}`. `verdict` is `proceed` \| `blocked` \| `needs_adjustment` \| `unresolved` \| **`no_return`** \| **`skipped`** — `no_return` being an absent sidecar after a dispatch that supplied an output path, which is never read as `proceed`, and `skipped` being a task with no `architecture/main.md` for the validator to fit against, which is a benign missing-input state and is deliberately NOT `unresolved`. `reason` is one line and is required for `no_return`, `unresolved` and `skipped`; this field is where that reason lives, and `commands/review.md` step 5.0 named this record before the field existed, which is the `notes` failure below in a second place. `null` on an entry that dispatched no validator. **Recorded here, adjudicated elsewhere** — the gate consequence is the `architecture-fit` `gates_run[]` entry in `_review.json` (§5.8), because this record never blocks. |
