...
3. **Review Required check (v4.1.0+).** Read `**Review Required:**` field from `project_state.md`:
   - **`true` OR (absent AND `completed/` empty):** Phase 4 must be `[x]`. Verify `<task>/_review.json` exists with `gate_specific.pr_ready: true` (or `gate_specific.overall_verdict: "bypassed"` with all `bypass_reason` populated — `bypass_reason` IS top level, hoisted there by `gate-audit-write.sh`, and the verdict is not). On missing/incomplete audit: print soft-warn ("`/review` did not run; gates not validated. Continue without `/review`? [y/N]") default `[N]`. User declines → exit; suggest `/ai-dev-assistant:review <task>`.
...
4. **Candidate-play surface (v3.15.0+).** Invoke `ai-dev-assistant:analysis-agent` in `play_candidates` mode (analyzes task artifacts + `git diff` for repeated decisions worth capturing), telling it to write its sidecar to `<task_folder>/_analysis-play_candidates.json`, and read the candidates off that file rather than out of its message, which is a channel an agent can finish having left empty. **An absent or unreadable sidecar is `no_return`** (`references/gate-audit-schema.md`), which is `decision: "insufficient_info"` here — say so and surface nothing, rather than letting a silent agent read as "this task repeated no decision worth capturing". For each candidate, prompt `[y]/[n]/[d]`. `[y]` hands off to `/playbook-capture`. Opt-out: `--no-play-candidates`.
...
- `/ai-dev-assistant:implement <task>` — continue Phase 3
- `/ai-dev-assistant:review <task>` — run Phase 4 gates before completing (v4.1.0+)
- `/ai-dev-assistant:upgrade-project` — set `**Review Required:**` explicitly on existing projects
