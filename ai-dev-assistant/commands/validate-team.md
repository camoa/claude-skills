...
```
/ai-dev-assistant:validate-team                         # run on current task, allow fallback
/ai-dev-assistant:validate-team <task-name>             # run on a specific task
/ai-dev-assistant:validate-team <task-name> --no-fallback   # refuse to run /validate:all if team spawn fails
```
