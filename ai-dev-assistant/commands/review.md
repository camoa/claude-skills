...
5. **Run hard-block gates sequentially.** For each: invoke flow inline (do NOT shell out). Capture per-gate envelope at `<task>/validations/latest/<gate>.json` per `references/validation-gate-result.md` v1.0. Add to `gates_run[]`. Order: tdd → solid → dry → security → guides → validate-playbook-adherence → skill-review (conditional) → plugin-validate (conditional).
   ...
   **5.0 Architecture-fit validation against the framework review recipe (recipe-resolution protocol).** Before the gate orchestration below runs the code-quality gates, validate the change against the documented architecture using the stack's review recipe. This is additive — it does NOT replace or weaken any hard-block gate below; it supplies the architecture-validator the framework-specific BLOCKING checks that the generic agent does not carry.

   - **Resolve the framework review method.** Follow the shared recipe-resolution protocol in `references/recipe-resolution.md` with `phase: review` and the active project's `<project_folder>` (resolved in step 2). That protocol invokes the `process-recipe-loader` skill, resolves each framework's review recipe (project_state-first, then source order, else `action:ask-user`), records the source in `project_state.md`, and defines how to follow each result: Read the `body_path` (never streamed), follow `verified:true` directly, surface `verified:false` for human review first, and on `action:ask-user` ask the user for a path or to research. Surface any loader `warnings[]` (for example `no_frameworks_defined`, `navigator_unavailable:<framework>`, `recipe_not_published:<framework>`) to the user. The COMMAND owns this resolution and injects the resolved recipe body into the validator's context; the `architecture-validator` agent stays generic and needs no Skill tool.
   - **Read the body and inject it verbatim.** For each framework result with `available:true`, Read its `body_path` with the Read tool (it is never streamed), gate it by `verified` (follow `verified:true`; surface `verified:false` for human go-ahead first), then dispatch `architecture-validator` with the Task tool and include the recipe body **verbatim** in the agent's prompt, inside the delimited block from `references/recipe-resolution.md` step 4 (`=== RESOLVED RECIPE (key=…, source=…, verified=…) === <body> === END RECIPE ===`). Reading `body_path` and dispatching the validator without including the body is a bug: the agent would have no framework-specific check set to follow. The validator keeps its BLOCK posture — a blocking architecture-fit or gate failure it reports is a hard-block result for this review, never softened to a warning.
   - **No body resolved → do not dispatch a method-less agent.** Dispatch `architecture-validator` **only** when a `body_path` resolved for the framework. On `no_frameworks_defined` tell the user to run `/upgrade-project` (or set Frameworks in `project_state.md`) and run the validator against the stack-neutral gates and `architecture/main.md` only (no fabricated framework rules); on `action:ask-user` ask the user for a path or to research and proceed per the answer; on a framework that resolved nothing skip it with a clear note. Per `references/recipe-resolution.md` step 6, the generic-agent dispatch needs a `body_path` to inject. This wiring never relaxes the hard-block gate orchestration that follows.
   ...
   **Default (change-scoped) — all modes except `--full-audit`:** From the step-4 merge-base diff (`git diff $(git merge-base "$BASE" HEAD)..HEAD --name-only` with the resolved `--base`, run from the review's cwd), filter to gate-relevant extensions. The extension set is the **union** of a framework-neutral language floor (`.php` `.js` `.mjs` `.cjs` `.ts` `.tsx` `.vue` — extensions that name no framework) **and** the active framework's code-quality extensions, reconstructed from the **same review recipe body already resolved and Read at step 5.0** (no second resolution): for each framework whose recipe resolved `available:true`, take its `## Code-quality extensions` declaration (the `code_quality_extensions` JSON list) and add those extensions to the filter set. When no recipe resolved (or a framework declares none), the neutral floor is used alone — a framework-specific file type with no recipe declaration is simply not scoped in, matching the agnostic-floor posture of the change-impact classifier. (When a framework's recipe declares extra suffixes — a templating extension plus that framework's own source-file types — the recipe restores them, so the effective filter covers that framework's full code surface rather than the neutral floor alone.) Write the filtered list to a temp file (e.g. `_review_changed_files.tmp`). Pass the temp-file path via `--files <tmpfile>` when invoking each of the four wrapper commands (`validate-tdd`, `validate-solid`, `validate-dry`, `validate-security`). If the filtered list is empty (a CSS/config/asset-only change), the underlying code-quality `--changed` mode handles the skip internally — no `/review`-side binary skip is needed. This skip is **not** a `--skip-<gate>` bypass (no `bypass_reason`), so by step-8 rule 4 it does **not** prevent `overall_verdict: pass`. Clean up the temp file after all four gates have run.

   **`--full-audit` mode:** Skip the diff-filter step entirely. Invoke all four gates with **no** `--files` argument — the whole-tree scan runs as in pre-v4.20 behavior. Use this to surface pre-existing whole-codebase debt rather than scope to the diff.

   `guides` is **not** covered by this mechanism (it has its own applicability check; CSS/SCSS/design-system changes ARE applicable and should cite the relevant guide); `validate-playbook-adherence` / `skill-review` / `plugin-validate` keep their own conditions. If the diff is empty (no committed change), the four gates receive an empty file list and each skips cleanly via the code-quality `--changed` mechanism.
...
- `/ai-dev-assistant:implement` (Phase 3) · `:complete` (archive; consumes `_review.json`) · `:validate-all` / `:validate-team` (invoked by `/review`) · `:upgrade-project` (set `**Review Required:**`) · `:audit-status` (audit visibility)
