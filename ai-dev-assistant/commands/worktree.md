...
**Under `--ddev-up`, a pinned `name:` is a HARD blocker, not a warning.** Auto-naming (the worktree dir → the DDEV project name) is what keeps the worktree's instance distinct from the main project's; a pinned `name:` makes both projects share one name and `ddev start` refuses the second (`project root is already set … refusing to change it`). So with `--ddev-up` and a `name:` set: do NOT offer `[c]ontinue` — halt. **THE fix is to remove the `name:` line from `.ddev/config.yaml` and commit** (then the worktree branches off the fixed commit and both the main checkout and the worktree auto-name distinctly). **Do NOT present `ddev config global --omit-project-name-by-default` as an alternative way out of THIS halt.** The halt fires precisely because the **tracked** `.ddev/config.yaml` carries `name:`, and a git worktree **inherits that same tracked file** — it still literally contains `name: <x>`, and an explicit `name:` in config.yaml **overrides** the machine-global omit default. So the flag leaves the colliding name in place: the operator re-runs `/worktree --ddev-up` and hits the **identical halt** — a dead-end loop. **Scope note:** `--omit-project-name-by-default` is a **machine-global** DDEV setting that, per the DDEV docs, changes only the *default* for **newly-`ddev config`'d** projects (`ddev config` stops writing a `name:`) — it never edits an existing committed `name:`. Mention it, if at all, only as a **forward-looking note** ("to stop a *future* `ddev config` from writing a `name:` at all"), never as the remedy for the committed `name:` that triggered this halt. `--no-ddev-check` does not bypass this (you cannot bring up a correctly-named isolated instance with a pinned name).
...
| `--ddev-up` AND `.ddev/config.yaml` has `name:` set | **Hard halt** (Step 5) — no `[c]ontinue`; **THE fix = remove the `name:` line + commit**, then re-run. Do NOT offer `--omit-project-name-by-default` as the way out — the worktree inherits the same tracked config.yaml and an explicit `name:` overrides the global omit default, so the flag can't clear a committed `name:` (dead-end loop); mention it only as a forward-looking note |
