...
```
/ai-dev-assistant:propose-epics
/ai-dev-assistant:propose-epics --only-in-progress    # (default; reserved flag for future extensions)
```
...
4. **Analyzes each candidate** — invokes `ai-dev-assistant:analysis-agent` (via Task tool, one subagent per candidate — they can run in parallel). **Hand each dispatch its own output path and read the verdict off disk:** tell it to write `<candidate task folder>/_analysis-folder.json` and take `decision`, `proposed_decomposition[]` and `schema_version` off that file, not out of the subagent's message; one file per candidate folder, so parallel dispatches cannot overwrite each other. Each file holds structured JSON per `references/analysis-agent-schema.md` v1.0. **An absent or unreadable sidecar is `no_return`** (`references/gate-audit-schema.md`), which is `decision: "insufficient_info"` — that candidate is presented as such at step 5 and named in step 7's failure line, never as a `keep_flat` nobody judged. **Normalize each candidate's file** through `${CLAUDE_PLUGIN_ROOT}/scripts/analysis-agent-normalize.sh "<candidate task folder>/_analysis-folder.json"` (the argument is **required** — called bare it prints its usage and normalizes nothing) as it returns, before the schema-version check, presentation, or any `/migrate-to-epic` call — it deterministically clamps `confidence` to `low` when `code_read:false` (schema invariant 2). **Read its exit code per candidate**: any non-zero exit means that candidate got no verdict, and the value says which fact to record beside `no_return` — `3` nothing was written, `1` text that is not JSON, `2` a payload that is not an answer. Present it as `insufficient_info`, never `keep_flat`.
...
6. **Accepted proposals** → invoke `/ai-dev-assistant:migrate-to-epic <task> --children "<proposed names>"` under the hood. Reports result.
...
2. **Parallel analysis** — Agent SDK Subagents model fits: spawn N subagents (one per candidate) via the Task tool. Each subagent invokes `ai-dev-assistant:analysis-agent` with that candidate's own output path; the parent reads each sidecar off disk and aggregates, counting an absent one as `no_return`. Reference: Claude Code Agent SDK Subagents docs [cite AS-1 in research.md].
...
- `/ai-dev-assistant:next` mentions this command when the project has multiple long-running flat tasks

## Related

- `analysis-agent` — the subagent this command spawns per candidate
- `/ai-dev-assistant:migrate-to-epic` — atomic primitive invoked on accept
