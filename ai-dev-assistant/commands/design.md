...
3. **Playbook load.** Run `${CLAUDE_PLUGIN_ROOT}/scripts/playbook-load-deterministic.sh <project_folder>`. Surface conflicts once-per-session per topic. Write `_playbook-load.json` audit. If a project-level `glossary.md` exists, read it for naming consistency (soft — never blocks; absent is fine).
