...
5. **Author architecture.md.**
   ...
   - **No body resolved → do not dispatch a method-less agent.** Dispatch the design agents **only** when a `body_path` resolved for the framework. On `no_frameworks_defined` tell the user to run `/upgrade-project` (or set Frameworks in `project_state.md`) and skip the recipe-driven architecture step gracefully; on `action:ask-user` ask the user for a path or to research and proceed per the answer; on a framework that resolved nothing skip it with a clear note. Per `references/recipe-resolution.md` step 6, the generic-agent dispatch needs a `body_path` to inject.
