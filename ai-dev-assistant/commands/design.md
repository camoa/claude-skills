...
2. **Dev-guides preflight (two-stage, v4.10.0+).** Same hybrid detection as `/research` step 3 (see that command for the full description).
   ...
   - Render the preflight prompt with `${CLAUDE_PLUGIN_ROOT}/scripts/prompt-render.sh dev-guides-preflight task_name=… methodology_floor=… matched_domain_guides=…` (Bash) and show exactly what it printed — no retyping, no relabelled groups, no heading above it. Block on `[c]/[a]/[n]` (default `[c]`; semantics unchanged). Write `_dev-guides-load.json` audit with `methodology_floor[]`, `catalog_candidates[]`, `matched_domain_guides[]`, `guides_actually_loaded[]`.
