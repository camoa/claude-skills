...
6. **Author research findings.** Invoke `contrib-researcher` agent + `core-pattern-finder` skill.
   ...
   - Add `## Coverage Mapping` H2 mapping each Research Question (and task-level AC if present) to the research that addresses it. **`## Coverage Mapping` MUST stay in the `research.md` hub** — the coverage-mapping gate reads only `research.md`; its rows may point into `research/<subject>.md` files. See `references/research-walkthrough.md` §"Output".
