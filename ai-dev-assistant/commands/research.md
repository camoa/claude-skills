...
2c. **Agentic-recipe discovery + gate (capability-class, v5.12.0+).** Follow the full protocol in `references/agentic-recipe-resolution.md` — do not re-describe its branches here. **Idempotent (but a `deferred` match must re-surface):** short-circuit this step only when this task's recorded decision is **terminal** — `project_state.md`'s `**Agentic Recipes:**` line (equivalently `_agentic-recipe.json`) records `decision` ∈ {`adopted`, `used_own`, `no_match`}. A `deferred` decision (recorded when an unattended run hit a match it couldn't prompt on) or no record at all → **re-enter the gate** so the deferred match is surfaced on this next attended run (contract step 3, the unattended branch). Do NOT short-circuit on mere `_agentic-recipe.json` existence — `deferred`/`no_match` also write it. Otherwise: invoke the `recipe-loader` skill (Skill tool) to decompose the task **Goal** into capability aspects and write `<task_folder>/coverage-map.json` (surface its `warnings[]`); then apply `references/agentic-recipe-resolution.md` steps 3–4 — the **hard gate with explicit escape**: a `kind:recipe` miss → proceed generically (`decision:"no_match"`); a `verified:true` match → **blocking** `[a]dopt` (default; recipe becomes the task spine) / `[o]` use-my-own (requires a free-text `reason`, `decision:"used_own"`); a `verified:false` match → **halt-and-escalate** for explicit go-ahead before any adoption. **Drift fail-open fix:** do NOT key the gate on `entry.verified` alone — if `coverage-map.json` `warnings[]` carries `recipe_body_unverified:<recipe_name>` for the matched recipe, the cached body drifted/is missing (attacker-seedable); treat it as **`verified:false` → halt-and-escalate** regardless of the entry's source-derived `verified`. An unattended run (`--headless`/loop) → do NOT block, record `decision:"deferred"`. Record the decision per step 4 in `project_state.md`'s `**Agentic Recipes:**` block AND `<task_folder>/_agentic-recipe.json` via `${CLAUDE_PLUGIN_ROOT}/scripts/gate-audit-write.sh "<task_folder>" agentic-recipe "<payload>"` (payload per `references/gate-audit-schema.md` §5.13). Never fabricate a `## Sequence`/`## Verifier`. **On `[a]dopt`, persist the body (step 4):** read the matched recipe's body from the navigator recipes cache (`.recipes[<recipe_name>].content`; re-invoke `recipe-loader`/navigator recipe-search for `<recipe_name>` if absent or sha-stale), **write it to `<task_folder>/adopted-recipe.md`** (Write tool), and record `body_path` = `<task_folder>/adopted-recipe.md` in `_agentic-recipe.json` (`null` for `used_own`/`deferred`/`no_match`). That persisted `adopted-recipe.md` is the durable, task-scoped spine `/implement` (its `## Sequence`) and `/review` (its `## Verifier`) Read downstream — **not** a navigator-served path.
