...
5. **Catalog-grounded code-change inference (v4.3.0+).** Skip with `--no-code-inference` (records `code_inference.suppressed_by_flag: true`).
   ...
   **c) Invoke `ai-dev-assistant:guides-matcher` agent** in `validation` mode with the union, the catalog path, optional `context_excerpts[]` from `implementation.md` Files Created/Modified, and `already_cited[]` from Step 4. Per `references/guides-matcher-schema.md`. **Hand it an output path and read the match off disk (v5.37.0+):** tell it to write `<task_folder>/_guides-match-validation.json` and take `matched_guides[]`, `unmatched_files[]` and `warnings[]` off that file as scalars, never out of the agent's message, which is a channel an agent can finish having left empty. **An absent or unreadable sidecar is `no_return`** (`references/gate-audit-schema.md`); (d) says what it costs. This gate does not itself resolve a process recipe, so it passes no `routing_hints[]`; the agent's neutral role buckets match generic conventions, and framework-specific config/template suffixes fall to `unmatched_files[]` — acceptable for a soft-nudge advisory.
   ...
   **On `no_return` the coverage-gap check is `unresolved`, not zero gaps.** Set `domain_coverage_gaps` to `null` — never `[]` — and add `code_inference.warnings: ["guides_matcher_no_return"]`. The Step 6 verdict rules are unchanged; what changes is the input they read. `null` does not satisfy the `pass` rule's `domain_coverage_gaps == []`, so a run whose matcher returned nothing cannot reach `pass` on a check that never ran; settle it at `warning` and name the unresolved check in the message. `[]` would be the claim that the changed files were matched against the catalog and every match is already cited, which is the one thing this gate exists to establish and the thing nobody did.
