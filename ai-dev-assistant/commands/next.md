...
```
/ai-dev-assistant:next                # Current project
/ai-dev-assistant:next my_project     # Specific project
```
...
```
Work-orders compiled (N total: X done, Y ready, Z needs_rework) — build via independent agents: `/ai-dev-assistant:run-work-orders <task>`
```
...
> 💡 This task has no scope contract (`alignment.md`). If scope has drifted or is unclear, consider running `/ai-dev-assistant:scope <task>` to retrofit a task-level alignment contract before continuing. (Optional — this is a one-line nudge, not a block.)
...
Command: /ai-dev-assistant:implement settings_form
```
...
Command: /ai-dev-assistant:research admin_dashboard
This will:
...
- `/ai-dev-assistant:status` - Full status overview
