...
2. **Project-state pass — gap detection.** Run `bash scripts/project-state-read.sh <project>`. Inspect emitted JSON. For `**Review Required:**` and `**Run Mode:**` (spine_memory), also direct-grep the dial line (`grep -i '^\*\*[Rr]eview [Rr]equired:\*\*'` / `grep -i '^\*\*[Rr]un [Mm]ode:\*\*'`) — the reader always emits `.runMode` (defaults `interactive`) so JSON-absence can't detect a missing *line*; gap fires only when the grep finds nothing (idempotent, never reads/trusts an existing autonomous value). Build `gaps[]` for: `**Code Path:**` (absent / `code_path_unknown`), `**Playbook Sets:**` (`playbook_sets_source: "default"` or `"default-empty"` = implicit; match both), `**User Playbook:**` + state (`unset`), `**Worktree By Default:**` (absent), `**Review Required:**` (absent), `**Run Mode:**` (line absent), `**Orchestration policy:**` (`<project>/orchestration-policy.json` absent beside `project_state.md`), `**Frameworks:**` (`frameworks == []` and codePath is known), `**Code Map:**` (reader `.codeMap` is `null` — never asked; a recorded `{"declined": true}` is an answer, not a gap), `**Visual Review:**` (reader `.visualReview` is `null`), **E2E preflight seam** (see below). **A new project-state field must be added in four places** — the reader, this gap list, the step-4 apply branch, and the step-3 prompt — or it is missed; Code Map and Visual Review both were.
...
4. **Project-state pass — apply via journal-backed atomic batch with file-lock (H4).** Before any write, acquire exclusive lock via `flock` on `<project>/.upgrade-project-journal.lock` (refuse with "another /upgrade-project run in progress" + exit 2 if held). Then create `<project>/.upgrade-project-journal.json` with planned operations + per-op `done: false`. For each gap:
   ...
   - `**Code Map:**` → invoke `/ai-dev-assistant:set-code-map` via `Skill` (a pointer to a map the user builds and owns; the framework never runs a mapping tool). Declining records `(none)`, which reads as answered and is never re-asked.
   - `**Task Rule:**` → invoke `/ai-dev-assistant:install-task-rule` via `Skill` (writes the "work here goes through a task" block into the code repository's own `CLAUDE.md`, where a session reads it as an instruction rather than as session-start context). Consented, never silent — it writes into the user's repo. Declining records `(none)`, which reads as answered and is never re-offered. Skip silently when codePath is unknown.
   - `**Visual Review:**` → report the gap and name `/setup-visual-regression`; do NOT write the line. Its Step 9 writes it while installing the harness, and a line pointing at a registry that does not exist is worse than none.
   - `**Frameworks:**` → skip silently when codePath is unknown or `code_path_unknown`. Otherwise resolve the framework with the cascade in `references/framework-resolution.md`: run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/framework-evidence.sh" "<codePath>"` for the facts (it identifies nothing and holds no framework list), name the stack yourself from those facts plus whatever the evidence says is worth reading, then run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/framework-support.sh" "<slug>"` per named framework to see what the catalogs carry for it. With one or more frameworks named, direct `Edit` insert `**Frameworks:** <slugs, comma-joined>` (e.g. `drupal, nextjs, claude-code-plugins`) after the last existing metadata field in the top block (same insertion approach as `**Worktree By Default:**`). When the cascade names none, skip silently. Never write an empty or placeholder line.
...
4b. **Recipe-adoption sweep — record the map, report the gaps** (canonical description; `project-initializer` step 10a runs it at creation too since v5.27.0) (runs after Step 4 once `**Frameworks:**` is known; part of project-level work, so it runs even with `--skip-tasks`; skipped when frameworks are empty). An eager on-ramp that resolves + **records** process-recipe `source=` decisions for all applicable phases (`research`, `design`, `implement`, `review`, `e2e-setup`, `visual-regression`) in one pass and prints a coverage map — **recording + visibility only**: it drives `process-recipe-loader` per phase (idempotent), never Reads/follows a body, never pre-caches, never approves `verified:false`, and never prompts (every miss = "no recipe yet"). Under `--dry-run` it reports what it would sweep and writes nothing. **Full mechanics — the guard, idempotency snapshot, the per-phase loop, the non-blocking miss mapping, and the coverage-map table format — are in `references/recipe-adoption-sweep.md`; follow that reference for this step.**
