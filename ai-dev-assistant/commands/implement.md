...
6. **Load context.**
   - **Follow an adopted agentic recipe's Sequence (capability-class, v5.12.0+).** If this task has an `adopted` agentic recipe (per `project_state.md`'s `**Agentic Recipes:**` block / `<task_folder>/_agentic-recipe.json`, written by `/research`), it is the implementation **spine**. Per `references/agentic-recipe-resolution.md` step 5: Read its body via the navigator-served `body_path` (never streamed/`curl`ed), **assemble its typed `## Input contract`** — derive what the project audit yields, ask the operator for policy fields, and **halt on any situation its contract doesn't cover (never guess)** — then follow its `## Sequence` as the build spine, honoring its `escalation_policy` halts. (`verified:false` never reaches here — `/research` step 3 escalated it first.) This sits **above** the framework process-recipe resolution below, which still supplies the stack-specific implement method.
