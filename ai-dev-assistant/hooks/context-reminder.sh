...
case "$CUR" in
  1)    CUR_LABEL="Phase 1: Research"       ; NEXT_CMD="/ai-dev-assistant:research $TASK" ;;
  2)    CUR_LABEL="Phase 2: Architecture"   ; NEXT_CMD="/ai-dev-assistant:design $TASK"   ;;
  3)    CUR_LABEL="Phase 3: Implementation" ; NEXT_CMD="/ai-dev-assistant:implement $TASK" ;;
  done) CUR_LABEL="All phases complete"     ; NEXT_CMD="/ai-dev-assistant:complete $TASK" ;;
esac
...
BLOCK=$(cat <<EOF
**ai-dev-assistant protocol is active on this task.** You are on \`$TASK\` — $CUR_LABEL. Phase sequencing applies (Research → Architecture → Implementation). Apply SOLID, TDD, and DRY to any code changes. Keep \`task.md\` \`## Phase Status\` checkboxes current as each phase progresses.
