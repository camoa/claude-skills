...
**An AI assistant for developers focused on doing development the right way** — sound process and best practices, not just raw speed. It guides AI through a disciplined **Research → Architecture → Implementation → Review** workflow: understand the problem before coding, reuse what already exists, follow your standards, and verify — and it *teaches* best practice as you go. This prevents the common AI pitfalls — jumping to code without understanding requirements, overlooking a pre-built library that already solves the problem, creating inconsistent architecture, or skipping tests. The orchestration engine is **stack-agnostic**; the components and guides it ships with today are **Drupal-flavored** (stack-neutral versions are in progress).
