...
| `/setup-e2e` | **(v4.12.0)** Resolve the `e2e-setup` process recipe for each project framework and follow it to stand up a behavioral E2E harness, scaffold `tests/e2e/`, seed the surface registry, and discover site journeys. Framework-agnostic (for a Drupal project the recipe installs ATK `^2.0` + Playwright and discovers journeys). Idempotent; `--add-journey` adds a single journey post-setup. |
...
See [CHANGELOG.md](./CHANGELOG.md) for full version history. Current version: **5.2.0**.
