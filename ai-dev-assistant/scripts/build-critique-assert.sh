...
if [ -e "$REC" ]; then
  ...
  #
  # GROWTH DURING REPAIR. The builder answered `concern`-level findings with new mechanism
  # rather than the minimum fix, and every new mechanism was fresh attack surface. A method
  # that did not exist before round 2's repair collected 38 of the 58 findings raised in
  # rounds 3 through 5. Production code grew 72% across the loop. Nothing asked whether a
  # repair was minimal, so nothing noticed that the repairs were the defect source.
  #
  ...
  # the first one.
  #
  # Neither is machine-checkable as "was this minimal" or "was this premature". Both are
  # checkable as "did you say so". A repair that grew the component says why it had to; a
  # finding that cannot be resolved yet says what it is waiting on, and stops blocking.
  ...
  UG_N=$(jq -r 'length' <<<"$UNJUSTIFIED_GROWTH")
  if [ "$UG_N" -gt 0 ]; then
    set_ev unjustified_growth_rounds "$UNJUSTIFIED_GROWTH"
    add_msg "$UG_N repair round(s) grew the component with no reason recorded; a repair that adds mechanism has to say why the minimum fix would not do"
    emit fail true "" 1
  fi
  ...
  if [ "$BD_N" -gt 0 ]; then
    ...
    add_msg "$BD_N deferred finding(s) name no blocked_on component; a deferral that does not say what it waits for is a finding nobody will return to"
    emit fail true "" 1
  fi
