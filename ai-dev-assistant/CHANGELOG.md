...
- **The four wrappers now resolve their verdict from the gate's JSON report, not its
  console output.** This is the actual fix, and the first attempt at it was decorative
  without it. The verdict tables are documented as "ordered; first match wins" and their
  first row matches `Explicit "PASS" / "✓"` — while `solid-check.sh` prints
  ...
  verdict. A coverage row added below that first row can never evaluate.
  ...
  `measured`, `skip_reason`, `tools_absent[]`, `tools_failed[]`, `tools_unmeasured[]` and
  `analyzers_ran`. The claim these files carried — "v1 uses heuristics because no stable
  JSON surface exists yet upstream" — was false when it was written, and is gone. Each
  wrapper now locates its report with `report-dir.sh --latest` rather than a hardcoded
  `.reports/`, which stopped being the default in code-quality-tools v3.9.6 and would have
  found nothing on a normal run.

- **The states that mean "nothing was checked" have rules, and they are checked first.**
  Each wrapper resolves report-present, then coverage, then findings, in that order. A
  ...
  benign `warning`. Zero coverage on a multi-analyzer gate is derived from the tool lists,
  **not** from `analyzers_ran == 0`: that counter is unreachable at zero for SOLID, whose
  always-on `\Drupal::` grep needs no binary and increments it, and it is absent entirely
  from `security-check.sh`'s standard path. `tools_failed[]` now disqualifies exactly as
  `tools_absent[]` does — a crashed analyzer produced no evidence either. Partial coverage
  is `warning`, naming what went unchecked, never `pass`.
...
- **Repair the minimum, and defer what cannot be answered yet.** Two rules with one cause, both
  ...
  *Growth during repair.* Each round the builder answered a `concern` with new mechanism instead
  ...
  exist before one round's repair collected 38 of the 58 findings raised in the three rounds
  after it. Production code grew 72% across the loop while behaviour stopped changing after
  round 3, and the criticals moved into the comments. Each repair round now records
  `repair_growth: {net_lines, reason}` against the previous round's checkpoint, and growth with
  no reason fails the gate. Growth is allowed; unexamined growth is not.

  *Findings nothing can answer yet.* "This method has no production caller" is true and
  unfixable when the caller is three components away in the build order. With nowhere to put it,
  the builder wrote a specification for the absent caller — that answer produced the next round's
  critical, and that answer produced the round after. Three of the four unanswerable findings in
  that build were self-generated by trying to close the first one. `wo-critic` now reports them
  in `deferred[]` as `{finding, blocked_on, why_now_is_wrong}`; they do not block, and they are
  re-checked once `blocked_on` exists. A deferral naming no `blocked_on` fails, because a finding
  nobody will return to is a dropped finding wearing a label.

  Neither is machine-checkable as "was this minimal" or "was this premature". Both are checkable
  as "did you say so", which is the standard the rest of this record already holds.
...
- **A round budget on the build-critique rung.** `[a]ddress` starts a new round on the component
  ...
  At the second blocking round on one component the loop halts: attended, the choice goes to the
  ...
  unrecorded one is not. A builder wrong twice running is the least reliable judge of
  whether the third attempt is different — the same reason the rung exists at all.
...
- **Optional `mechanism_hints` task frontmatter** (`[{approach, status}]`, read via `fm-helpers.sh`), read-if-present with prose-extraction as the floor — **decoupled** from the concurrent converter work (G-CONV-40/50): the engine challenges any task regardless of source; there is intentionally no `/create-recipe` / auto-create wiring. **Seam contract:** the converter handshake is via body tags (`mechanism: suggested`, `adopt_recipe: <name>` — the latter a re-verified resolver hint), NOT frontmatter; `mechanism: required` is human-only (no converter producer). `mechanisms_hash` is engine-owned, computed by the deterministic `scripts/mechanisms-hash.sh` kernel (not hashed by hand).
...
- **GAP F (HIGH — loop-breaking for a whole task class): `wo-review-snapshot.sh` crashed on any null/absent `envelope_path`.** A skipped hard-block gate legitimately has no envelope (`envelope_path` null/absent is schema-valid, §5.8). The pointer-rewrite guard type-checked the *coalesced* value (`(.envelope_path? // "") | type == "string"` → `""` passes) but then ran `gsub` on the *raw* `null` → `jq: null cannot be matched` → `rewrite_failed`, so the per-WO `wo-NN._review.json` was never written and the WO could not settle `done`. On any CSS/SCSS/asset/config-only WO all four code-quality gates skip ⇒ at least one null `envelope_path` ⇒ the run-loop was effectively **broken for every styling/theme/token work-order**. Fix is the one-line raw-type guard `(.envelope_path | type) == "string"` (null/absent → type `null` → `else`, left untouched — there is no `latest/` pointer to rewrite). Regression test added: a mixed `gates_run` (string path + explicit null + key-absent) asserts `ok:true` and that only the string path is rewritten (verified it fails on the old guard).
- **GAP E (`work-order-builder` built blind on `../`-relative WO refs).** The builder spawned with `cwd = code worktree` and only a `BUILD ROOT` anchor (correct for the body's worktree-relative `## Files to touch`), but a WO body also carries **memory-repo-relative** refs — prose pointers to authoritative inputs (`Read ../task.md …`) and a frontmatter `coverage_ref: ../coverage-map.json` — that resolve against the **WO file's own dir**, a different repo. A fresh builder resolved `../task.md` from cwd=worktree → a nonexistent path and built blind on every delegated value, surfaced only downstream at the review/critique rung. Fix: the atom now passes a **second** trusted-runtime anchor, `WO_DIR (resolve any ../ reference against this absolute path, NOT against BUILD ROOT): <abs>`, computed from the WO file's dir. New `work-order-builder-contract-spec.sh` guards both anchors + the fresh-context/leaf invariants.
- **GAP D (`/worktree --ddev-up` offered a non-working fix for a committed `name:`).** The pinned-`name:` hard-halt presented `ddev config global --omit-project-name-by-default` as a co-equal way out, but a git worktree **inherits the same tracked `.ddev/config.yaml`** and an explicit `name:` **overrides** the machine-global omit default — so the flag leaves the colliding name in place and re-running hits the identical halt (a dead-end loop). The halt now presents **`name:`-line removal + commit as THE fix** and demotes the omit-default flag to a forward-looking note (stop a *future* `ddev config` from writing a `name:`), in both the Step 5 prose and the error-cases table. Regression assertion added to `ddev-worktree-spec.sh`.
...
### Fixed — GAP C: playbook-adherence vacuous skip read as coverage
A project that **declares** a playbook set which **resolves to 0 plays** (the set is a name with no cached plays, and no local user playbook) made `validate-playbook-adherence` skip silently — indistinguishable from a genuine "nothing to check," so the skip read as coverage. Same *silent-vacuous-degrade* class as GAP B. Now: the deterministic loader emits `playbook_sets_declared_zero_local_plays` when a declared (non-`none`) set contributes 0 concrete plays, and the gate records **why** it skipped — `declared_playbook_resolved_zero_plays` (vacuous: surfaces a "this skip is **NOT coverage**" message) vs `no_playbook_declared` (benign no-op). An intentional `**Playbook Sets:** none` opt-out is not flagged.
