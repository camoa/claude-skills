...
- **Faithful design rendering** — the renderer's layout model carries a real design: per-element font size / weight / family, explicit colours, rounded rectangles, ellipses, a mono font, and uneven-stop gradients. The scaffolder also drops the API's seeded blank default slide.
- **`slides-renderer` skill** — the Google Slides analog of `visual-content`: composes a resolution-independent `LayoutSpec` from *any* template's `canvas-philosophy.md` + `template.md` and renders it via the Slides API. Generic — no per-template code. `/presentation`'s Google Slides target invokes it. `scripts/slides/src/community-talk-layout.ts` is a committed worked example (the `community-talk` template composed) + test fixture; `examples/render-community-talk.mjs` renders it.
