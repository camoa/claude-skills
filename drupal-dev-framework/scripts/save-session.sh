#!/usr/bin/env bash
# save-session.sh — drupal-dev-framework session persistence.
#
# Pure bash. No AI. No network. Safe to run unconditionally at session exit.
#
# Two entry points converge on this script:
#   - SessionEnd hook (wired by /drupal-dev-framework:install-remembrance-hook)
#     runs it directly on every session exit. No judgement — safety net.
#   - /drupal-dev-framework:save-session slash command runs it after Claude
...
if [ "$CHANGED_COUNT" -gt 0 ]; then
  echo "drupal-dev-framework: ${CHANGED_COUNT} markdown file(s) in task '${TASK:-?}' changed since the last save (${CHANGED_LIST}). Run /drupal-dev-framework:save-session to persist in-flight state with a review pass." >&2
fi
