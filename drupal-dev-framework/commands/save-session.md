---
description: "Persist in-flight drupal-dev-framework session state before ending work. Claude reviews the active task for un-written progress, then runs scripts/save-session.sh to stamp the session file and audit JSONs. The AI-judgement entry point — the SessionEnd hook runs the same script unconditionally as a scripted safety net. Introduced v4.5.0."
...
Persist the current drupal-dev-framework session before you stop working.
...
- `/drupal-dev-framework:install-remembrance-hook` — wires the `SessionStart`
  primer and the `SessionEnd` save-session hook into a project.
- `/drupal-dev-framework:next` — resume work and see the current phase/task.
