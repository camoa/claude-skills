...
5. **Run hard-block gates sequentially.** For each: invoke flow inline (do NOT shell out). Capture per-gate envelope at `<task>/validations/latest/<gate>.json` per `references/validation-gate-result.md` v1.0. Add to `gates_run[]`. Order: tdd → solid → dry → security → guides → validate-playbook-adherence → skill-review (conditional) → plugin-validate (conditional).
   ...
   **5a. Diff-type pre-filter for the PHP/JS-centric gates (`tdd` / `solid` / `dry` / `security`).** These four gates run the underlying code-quality tools against the project's **code**, not the diff — so on a change that touches no executable code they would re-surface *pre-existing* codebase debt and **false-fail a CSS/config/asset-only work-order**. Before invoking each of `tdd`/`solid`/`dry`/`security`, inspect the step-4 merge-base diff (`git diff $(git merge-base "$BASE" HEAD)..HEAD --name-only` with the resolved `--base`, run from the review's cwd — which for a loop run is the **worktree**). If that diff contains **zero** files whose extension is PHP/JS/TS/Twig (`.php` `.module` `.inc` `.install` `.profile` `.theme` `.engine` `.js` `.mjs` `.cjs` `.ts` `.tsx` `.vue` `.twig`), the gate is **N/A for this change**: write its envelope with `verdict: "skipped"`, `unresolved: false`, `messages: ["no PHP/JS/Twig files in diff; gate N/A for CSS/config/asset-only change"]`, and move on **without** invoking the underlying flow. This is a benign N/A skip (NOT a `--skip-<gate>` bypass — no `bypass_reason`), so by step-8 rule 4 it does **not** prevent `overall_verdict: pass`. `guides` is **not** pre-filtered (it has its own applicability check; CSS/SCSS/design-system changes ARE applicable and should cite the relevant guide); `validate-playbook-adherence` / `skill-review` / `plugin-validate` keep their own conditions. If the diff is empty (no committed change) the four gates still skip N/A, but the review's own dirty-tree / no-change handling governs the overall verdict.
