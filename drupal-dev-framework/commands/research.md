...
5. **(v3.12.0+)** After the `decision` branch completes (regardless of outcome unless the task became an epic), inspect `signals_used[]` for `scope_contract_recommended`. If present, print soft-nudge:
   ...
   On `[y]` → invoke `/drupal-dev-framework:scope <task_name>` inline; after it completes, continue with the standard research flow.
...
3. If user says `[y]`, invoke `/drupal-dev-framework:scope <task_name> --phase 1` inline. After it returns, continue with research.
