...
Tutorial-depth reference for the `/drupal-dev-framework:implement` command. The runtime command body (see `commands/implement.md`) is a terse imperative summary; this file documents every sub-step, rationale, version history, and worked example in full.
