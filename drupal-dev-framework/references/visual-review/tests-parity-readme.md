...
Playwright, and `pixelmatch`. Set up by `/drupal-dev-framework:setup-visual-parity`.
...
/drupal-dev-framework:validate:visual-parity

# All viewports:
/drupal-dev-framework:validate:visual-parity --all-viewports
```

Parity also auto-runs (soft) inside `/drupal-dev-framework:review` on
