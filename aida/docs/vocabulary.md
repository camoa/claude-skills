...
|---|---|---|
| scope, research, design, implement, review, completion | the six stages, named as their skills are | |
...
| check | one blocking check a stage runs | `gate`, `validate`, `validation`. `audit` is not banned: it is the review action that lists how each verdict came about |
