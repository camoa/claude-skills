...
3. **Six stages, in order.** Every task runs through
   [scope](scope.md), [research](research.md), [design](design.md),
   [implementation](implementation.md), [review](review.md), and completion, which is
   [finishing a task](finishing.md).

The process lives in AIDA. The knowledge about your stack lives in sources AIDA reads, so the
same six stages work for Drupal, Go, Python, or a Claude plugin. See
...
| You want your own rules followed on every task | [Playbooks](playbooks.md) |
