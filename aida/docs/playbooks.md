...
Plays come from three places, in precedence order:
...
The two files share one format, and a version 5 playbook loads unchanged. A `## <Domain>`
...
`playbooks load` merges the two files with it into one record in the task folder. The record
marks each source `loaded`, `absent`, `empty` or `unreachable`, with a count. So a missing file
and a set the catalog could not serve never read as "nothing to follow".
