...
- `"kind":"new"`: a task with its own `task.json`. Carries `id`, `state` (`new` or
  ...
  record. An open task reading `passed` or `failed` is reviewed, and completion closes it. It
  also carries `worktree`: the path of the task's own git worktree, or `none` for a task made
  before every task had one.
